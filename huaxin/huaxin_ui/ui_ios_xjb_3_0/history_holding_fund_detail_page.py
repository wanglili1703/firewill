...
class HistoryHoldingFundDetailPage(PageObject):
    def __init__(self, web_driver):
        super(HistoryHoldingFundDetailPage, self).__init__(web_driver)
        self.elements_exist(*current_page)

    @robot_log
    def verify_history_fund_holding_detail(self, product_name):
        self.assert_values(product_name, self.get_text("//UIAStaticText[@label='%s']" % product_name))
        self.assert_values('收益分配日期', self.get_text("//UIAStaticText[@label='收益分配日期']"))
